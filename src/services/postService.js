export const getAllPosts = () => {
  return fetch(
    "http://localhost:8088/posts?_expand=user&_expand=topic&_expand=user&_embed=likedPosts",
  ).then((res) => res.json());
};
...
export const getPostById = (id) => {
  return fetch(
    `http://localhost:8088/posts/${id}?_expand=user&_expand=topic&_embed=likedPosts`,
  ).then((res) => res.json());
};
